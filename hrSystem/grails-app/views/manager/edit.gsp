...
<html>
    ...
    <body>
        <a href="#edit-manager" class="skip" tabindex="-1"><g:message code="default.link.skip.label" default="Skip to content&hellip;"/></a>
        ...
        <div id="edit-manager" class="content scaffold-edit" role="main">
            <h1><g:message code="default.edit.label" args="[entityName]" /></h1>
            <g:if test="${flash.message}">
            <div class="message" role="status">${flash.message}</div>
            </g:if>
            <g:hasErrors bean="${this.manager}">
            <ul class="errors" role="alert">
                <g:eachError bean="${this.manager}" var="error">
                <li <g:if test="${error in org.springframework.validation.FieldError}">data-field-id="${error.field}"</g:if>><g:message error="${error}"/></li>
                </g:eachError>
            </ul>
            </g:hasErrors>
            <g:form resource="${this.manager}" method="PUT">
                <g:hiddenField name="version" value="${this.manager?.version}" />
                <fieldset class="form">
                    <f:all bean="manager"/>
                </fieldset>
